...
object Aggregation {
  ...
  def main(args: Array[String]) {
    ...
    val flatColumn =vars.getOrDefault("flatColumn","Description")
    ...
    if (oldDF!=null&&newDF!=null&& !oldDF.rdd.isEmpty() && !newDF.rdd.isEmpty()) {
      ...
      val deletedOrUpdate = oldDF0.except(newDF0);
      val addedOrUpdate = newDF0.except(oldDF0);
      val deletedOrUpdateUid = oldDF0.except(newDF0).select("uid")
      val addedOrUpdateUid = newDF0.except(oldDF0).select("uid")
      
      val updateUid = deletedOrUpdateUid.intersect(newDF0.select("uid")).withColumnRenamed("uid", "uuid")
